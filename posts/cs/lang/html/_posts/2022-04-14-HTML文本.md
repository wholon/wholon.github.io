...
本节的任务是编写一个带超链接的`HTML`页面，链接包含页面内导航链接和跳转其他网页的链接。显示效果如下：

   ![img](https://holon-image.oss-cn-beijing.aliyuncs.com/20220418200211H50z6g.png)  
...
本节任务是完成一个格式丰富的文章网页，你将通过本节学习标题、段落、文本格式化和引用等，与文本相关的标签。
...
例`1`和例`2`唯一的不同在于，例`2`中我们在段落内容中换行了。那么，在实际显示时，会显示换行吗？
